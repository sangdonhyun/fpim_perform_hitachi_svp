...
class FpimDb():
    ...
    def table_create(self):
        day_format = datetime.datetime.now().strftime('y%Ym%md%d')
        ins_date = datetime.datetime.now().strftime('%Y-%m-%d')
        tbname = 'monitor.perform_stg_{}'.format(day_format)
        query = "select count(*) from pg_tables where tablename ='perform_stg_{}';".format(day_format)

        rows=self.get_query_list(query)
        try:
            cnt = rows[0][0]
        except:
            cnt = 0
        #print 'today table cnt :',cnt
        if cnt == 0:
            query  = """

CREATE TABLE monitor.perform_stg_{DAY_FORMAT}
(
  CONSTRAINT perform_stg_{DAY_FORMAT}_ins_date_check CHECK (ins_date = '{INS_DATE}'::date)
)
INHERITS (monitor.perform_stg)
WITH (
  OIDS=FALSE
);
ALTER TABLE monitor.perform_stg_{DAY_FORMAT}
  OWNER TO postgres;

""".format(DAY_FORMAT=day_format,INS_DATE=ins_date)
            print query
...
#             print "Number of rows updated: %d" % cur.rowcount
    ...
    def dbListInsert(self, dicList, table='monotir.pm_auto_isilon_info'):
...
#             print "Number of rows updated: %d" % cur.rowcount
    ...
    def get_query_list(self, query_string):
...
#         self.logger = self.com.flog()
        ...
        # self.conn_string = "host='10.38.201.222' dbname='qweb' user='webuser' password='qw19850802@'"
        self.conn_string = self.getConnStr()
