...
password = kes2719!
    ...
    def getAvg(self,datehour,target):
        portVal={}
        """
        lrange "hitachi_latest_time_000000000ALJ2503G08G_2019-12-09 10" 0 -1
        ...
        """
        lastkey='hitachi_latest_time_{}_{}'.format(self.serial,datehour)
        print lastkey
...
4) "throughput"
    ...
    def getFlagList(self):
        flagList=self.r.lrange('hitachi_flag_list',0,-1)
        return  flagList

    def set_avg_day(self, day_str):
        query = """delete FROM monitor.perform_stg psymd WHERE ins_date = '{}' AND device_type='hitachi' AND char_length(check_date) = '10'  
        """.format(day_str)
...
SELECT ins_date,ins_date as check_date,device_type ,flag_1, flag_2,'' AS flag_3, flag_4,max(cols_value_max) cols_value_max ,avg(cols_value_avg::float) cols_value_avg  ,max(cols_max_date) AS cols_max_date,mdb_key
    FROM monitor.perform_stg psymd WHERE ins_date = '{}' AND device_type='hitachi' AND char_length(check_date) = '13' 
    ...
    def redis_set(self):
        ...
        self.r.lrange('hitachi_device_list',0,-1)
        device_set=self.r.lrange('hitachi_device_list',0,-1)
        dicList=[]
        tot = len(device_set)
        cnt = 1
        #print device_set
